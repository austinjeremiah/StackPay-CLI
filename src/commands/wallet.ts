...
export async function walletCreate(options: { network?: string; force?: boolean }) {
  ...
  try {
    ...
    console.log(chalk.bold.green('  ✅ stackspay Wallet (Seller)'));
    console.log(chalk.bold('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'));
    console.log(chalk.cyan('  Address  :'), chalk.white(wallet.address));
    console.log(chalk.cyan('  Network  :'), chalk.white(wallet.network));
    console.log(chalk.cyan('  Saved at :'), chalk.gray(`~/.stackspay/wallet.json`));
    console.log(chalk.bold('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'));
    console.log('');
...
export async function walletCreateBuyer(options: { force?: boolean }) {
  ...
  console.log(chalk.bold('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'));
  console.log('');
  console.log(chalk.yellow('💡 Fund this buyer wallet from faucet:'));
  console.log(chalk.gray(`   https://explorer.hiro.so/sandbox/faucet?chain=testnet`));
  console.log(chalk.gray(`   Address: ${wallet.address}`));
  console.log('');
  console.log(chalk.green('Then run: stackspay pay http://localhost:3000/run'));
}

export async function walletBalance() {
  let wallet: any;
  try { wallet = loadWallet(); }
  catch (err: any) { console.error(chalk.red(err.message)); return; }
...
async function fetchBalance(address: string, network: string): Promise<{ stx: string }> {
  return new Promise((resolve, reject) => {
    ...
    https.get({ hostname: host, path: `/v2/accounts/${address}`, headers: { Accept: 'application/json' } }, (res) => {
      let data = '';
      res.on('data', (c) => data += c);
      res.on('end', () => {
        try {
          const json = JSON.parse(data);
          const stx = (parseInt(json.balance, 16) / 1_000_000).toFixed(6);
          resolve({ stx });
        } catch (e) { reject(e); }
      });
    }).on('error', reject);
